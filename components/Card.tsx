...
const Card: React.FC<CardProps> = ({ title, children, className, action, onClick }) => {
  return (
    <div 
      onClick={onClick}
      className={`relative bg-white/90 dark:bg-zinc-900/60 backdrop-blur-xl rounded-[2rem] shadow-sm md:shadow-md border border-zinc-200 dark:border-zinc-800 group overflow-hidden transition-all duration-300 hover:shadow-lg dark:hover:shadow-glass-hover hover:bg-white dark:hover:bg-zinc-900/80 hover:-translate-y-1 ${onClick ? 'cursor-pointer' : ''} ${className}`}
    >
      
      {/* Rim Light Gradient on Top Border - Adapts to Theme */}
      <div className="absolute top-0 inset-x-0 h-[1px] bg-gradient-to-r from-transparent via-brand-secondary/30 dark:via-emerald-500/50 to-transparent opacity-50 group-hover:opacity-100 transition-opacity"></div>
